...
class HtmlReport implements ReportType {
	...
	public void initializeTestLog() {
		...
		if ((reportSettings.getIsAlexaTestCase())) {
	...
	public void addTestLogTableHeadings() {
		...
		try {
			...
			if (reportSettings.getisWebAutomation()
					&& Boolean.parseBoolean(properties.getProperty("InvokePerfInsightEngineForPerformance"))) {
				testLogTableHeading = "\t\t\t <thead> \n" + "\t\t\t\t <tr class='heading'> \n"
						...
						+ "\t\t\t\t\t <th>ScreenShot</th> \n" + "\t\t\t\t </tr> \n" + "\t\t\t </thead> \n\n";
			} else if ((reportSettings.getIsAlexaTestCase())) {
	...
	@Override
	public void updateTestLog(String stepNumber, String stepName, String stepDescription, Status stepStatus,
			String screenShotName, Map<String, Object> perfObject) {
		try {
			BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(testLogPath, true));
			...
			case FAIL:
				if (reportSettings.shouldTakeScreenshotFailedStep()) {
					testStepRow += getTestStepWithScreenshot(stepDescription, stepStatus, screenShotName, perfObject);
				} else {
					testStepRow += getTestStepWithoutScreenshot(stepDescription, stepStatus, perfObject);
				}
				break;
	...
	@Override
	public void updateTestLog(String stepNumber, String utterances, String expectedValue, String actualValue,
			Status stepStatus) {
		try {
			BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(testLogPath, true));
			...
			String testStepRow = "\t\t\t\t <tr class='content' id='" + currentSection + currentSubSection
					+ currentContentNumber + "'> \n" + "\t\t\t\t\t <td>" + stepNumber + "</td> \n"
					+ "\t\t\t\t\t <td class='justified'>" + utterances + "</td> \n";
			currentContentNumber++;

			switch (stepStatus) {
			case PASS:

				testStepRow += getAlexaTestStep(expectedValue, actualValue, stepStatus);
	...
	private String getAlexaTestStep(String utterances, String actualValue, Status stepStatus) {
		String testStepRow;

		testStepRow = "\t\t\t\t\t <td class='justified'>" + utterances + "</td> \n"
	...
	private String getTestStepWithScreenshot(String stepDescription, Status stepStatus, String screenShotName,
		...
		if (reportSettings.shouldLinkScreenshotsToTestLog()) {
			testStepRow = "\t\t\t\t\t <td class='justified'>" + stepDescription + "</td> \n" + "\t\t\t\t\t <td class='"
					+ stepStatus.toString().toLowerCase() + "'>" + stepStatus + "</td> \n" + "\t\t\t\t\t <td>"
					+ perfReportValues.get(0) + "</td> \n" + "\t\t\t\t\t <td>" + perfReportValues.get(1) + "</td> \n"
					+ "\t\t\t\t\t <td>" + perfReportValues.get(2) + "</td> \n" + "\t\t\t\t\t <td>" + "<a href="
					+ perfReportValues.get(3) + Util.runID + " target='about_blank'>Link</a>" + "</td> \n"
					+ "\t\t\t\t\t <td>" + "<small>" + Util.getCurrentFormattedTime(reportSettings.getDateFormatString())
		...
		try {
			...
			if (!(Util.runID == null)
				...
				resultSummaryFooter = "\t\t\t </tbody> \n" + "\t\t </table> \n\n" + "\t\t <table id='footer'> \n"
						...
						+ "\t\t\t\t </tr> \n" + addPerformanceFooter() + "\t\t\t\t <tr class='heading'> \n"
						+ "\t\t\t\t\t <th colspan='4'>Total Duration: " + totalExecutionTime + "</th> \n"
						+ "\t\t\t\t </tr> \n" + "\t\t\t\t <tr class='subheading'> \n"
						+ "\t\t\t\t\t <td class='pass'>&nbsp;Modules passed</td> \n"
						+ "\t\t\t\t\t <td class='pass'>&nbsp;: " + nTestsPassed + "</td> \n"
						+ "\t\t\t\t\t <td class='fail'>&nbsp;Modules failed</td> \n"
	...
	private List<String> validatenGetValues(Map<String, Object> perfObj) {
		...
		if (perfObj.get("UploadStatus") == null) {
			...
			double totalTime = Double.parseDouble(perfObj.get("totalTime").toString());
			double serverTime = Double.parseDouble(perfObj.get("serverTime").toString());
			...
			values.add(reportSettings.getTransactionUrl() + "&" + "Transaction=" + perfObj.get("txnName").toString()
					+ "&" + "RunID=");
		} else {
	...
	private String addPerformanceFooter() {
		...
		String resultSummarySubHeading1 = "\t\t\t\t <tr class='content'> \n" + "\t\t\t\t\t <th>&nbsp;"
				+ "Performance ProjectName".replace(" ", "&nbsp;") + "</th> \n" + "\t\t\t\t\t <th>&nbsp;"
				+ perfproperties.getProperty("projectName").replace(" ", "&nbsp;") + "</th> \n"
				+ "\t\t\t\t\t <th>&nbsp;" + "Performance ClientName".replace(" ", "&nbsp;") + "</th> \n"
				+ "\t\t\t\t\t <th>&nbsp;" + perfproperties.getProperty("clientName").replace(" ", "&nbsp;") + "</th> \n"
				+ "\t\t\t\t </tr> \n";

		String resultSummarySubHeading2 = "\t\t\t\t <tr class='content'> \n" + "\t\t\t\t\t <th>&nbsp;"
				+ "Performance ScenarioName".replace(" ", "&nbsp;") + "</th> \n" + "\t\t\t\t\t <th>&nbsp;"
				+ perfproperties.getProperty("scenarioName").replace(" ", "&nbsp;") + "</th> \n"
				+ "\t\t\t\t\t <th>&nbsp;" + "Performance Summary Report".replace(" ", "&nbsp;") + "</th> \n"
				+ "\t\t\t\t\t <th>&nbsp;" + "<a href=" + reportSettings.getPerformanceSummaryReportUrl() + Util.runID
